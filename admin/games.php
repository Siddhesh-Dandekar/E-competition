...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $cordinator_name = trim($_POST['cordinator_name']);

  // Validation and sanitization (optional, add checks as needed)
  if (empty($game_name) || empty($cordinator_name)) {
    $message = "Please fill in both game name and coordinator name.";
  } else {
    // ... (further validation logic if needed) ...

    // Prepare SQL statement with placeholders
    $sql = "INSERT INTO games (game_name, cordinator_name) VALUES (?, ?)";
    $stmt = $conn->prepare($sql);

    // Bind parameters with sanitized data
    $stmt->bind_param("ss", $game_name, $cordinator_name);
                ...
                <div class="flex flex-col flex-1 overflow-hidden">
                    ...
                    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
                        <div class="container px-6 py-8 mx-auto">
                            <h3 class="text-3xl font-medium text-gray-700">Add Games</h3>

                            <section class="text-gray-900 body-font">
                                <div class="container px-5 py-10 mx-auto flex flex-wrap items-center">
                                    <div
                                        class="lg:w-2/6 md:w-1/2 bg-white rounded-lg p-8 flex flex-col md: w-full mt-10 md:mt-0">
                                        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                                        <div class="relative mb-4">
                                            <label for="full-name" class="leading-7 text-base text-gray-600">Game Name</label>
                                            <input type="text" name="game_name" id="game_name" required
                                                class="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                        </div>
                                        <div class="relative mb-4">
                                            <label for="email" class="leading-7 text-base text-gray-600">Cordinator Name</label>
                                            <input type="text" name="cordinator_name" id="cordinator_name" required
                                                class="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                        </div>
                                        <button
